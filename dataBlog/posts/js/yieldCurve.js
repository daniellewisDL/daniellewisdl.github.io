...
const bannerGroupText = bannerGroup.append("text").attr("x", 80).attr("y", 35).attr("fill", "white").text("D3 Yield Curve").attr("font-size", 25)
...
function go() {
    ...
    Promise.all(promises).then(results=>{
        ...
        const yScale = d3.scaleLinear().domain([minYield, maxYield]).range([chartAreaHeight-50, 50])

        // Set x scale

        const xScale = d3.scaleBand().domain(columns).range([50, 800])

        
        const yieldCurveGroup = chartAreaGroup.append("g").attr("id", "yieldCurveGroup").attr("transform", `translate(100, 0)`)
        ...
        const interactiveLabelsGroup = chartAreaGroup.append("g").attr("id", "interactiveLabels").attr("transform", `translate(1100, 300)`)
        ...
        const xAxisGroup = chartAreaGroup.append("g").attr("id", "xAxisGroup").attr("transform", `translate(${100-xScale.bandwidth()/2}, ${chartAreaHeight-50})`).call(xAxis)
        xAxisGroup.select(".domain").remove()
        xAxisGroup.selectAll("line").remove()
        xAxisGroup.selectAll("text").attr("fill", accentColor).attr("font-size", 10)
        const yAxisGroup = chartAreaGroup.append("g").attr("id", "yAxisGroup").attr("transform", `translate(100, 0)`).call(yAxis)
