...
(function () {
  ...
  const ENEMY_FRAMES = { convict: 6, bobby: 4, turnkey: 4, bride: 4, ghost: 4, shadow: 4, rat: 2, stalker: 4 };
  for (const e in ENEMY_FRAMES)
    for (let i = 1; i <= ENEMY_FRAMES[e]; i++)
      MANIFEST[e + i] = ['assets/chars/' + e + i + '.png', false];
  // hazard + fixture art (pre-keyed)
  for (const k of ['trap1', 'trap2', 'wave1', 'wave2', 'wave3', 'wave4', 'coffin', 'masonry',
                   'clockface', 'lampoff', 'lampon', 'gateclosed', 'gateopen', 'envelope',
                   'board1', 'board2', 'board3', 'bonfire1', 'bonfire2', 'bonfire3', 'bonfire4'])
    MANIFEST[k] = ['assets/props/' + k + '.png', false];
  for (let a = 1; a <= 4; a++)
  ...
  for (const k of ['crate', 'barrel', 'candle', 'clock', 'lamp', 'wreath', 'bones', 'reeds',
                   'cake', 'bars', 'chimney', 'books', 'desk', 'chair', 'mast', 'wheel', 'post'])
    MANIFEST['prop_' + k] = ['assets/props/' + k + '.png', false];

  const A = UW.assets = {
    img: {},
    get(k) { return this.img[k] || null; },
    // frame-set helper: all-or-nothing so animations never mix art styles
    frames(name, n) {
      const arr = [];
      for (let i = 1; i <= n; i++) {
        const im = this.img[name + i];
        if (!im) return null;
        arr.push(im);
      }
      return arr;
