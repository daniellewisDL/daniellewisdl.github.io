...
(function () {
  ...
  UW.Enemy = class {
    ...
    update(dt, p) {
      ...
      switch (k) {
        ...
        case 'wave': {
          ...
          // ride the deck: settle onto the ground height under the crest
          const gen = this.world.gen;
          const c = UW.clamp(Math.floor(this.x / TS), 0, gen.W - 1);
          this.y += (gen.groundY[c] * TS - this.y) * Math.min(1, dt * 10);
    ...
    draw(ctx, cam) {
      ...
      // external frame, bottom-centre anchored; sheet art faces right, so flip when dir < 0
      ...
      switch (k) {
        ...
        case 'trap': {
          const img = UW.assets.get((this.cool || 0) > 0.9 ? 'trap2' : 'trap1'); // snapped shut after a grab
          if (img) drawE(img, 8, 0, 1, false);
          else {
        ...
        case 'coffin': case 'collapse': {
          ...
          const img = UW.assets.get(k === 'coffin' ? 'coffin' : 'masonry');
          if (img) {
            const dh = k === 'coffin' ? 18 : 12, dw = dh * img.width / img.height;
            ctx.drawImage(img, Math.round(X - dw / 2), Math.round(FY - dh / 2), Math.round(dw), dh);
          } else if (k === 'coffin') {
        ...
        case 'clockface': {
          const CY = Y - 30;
          const img = UW.assets.get('clockface');
          if (img) ctx.drawImage(img, X - 6, CY - 6, 12, 12);
          else {
        ...
        case 'wave': {
          const fr = AF('wave', 4);
          if (fr) drawE(fr[Math.floor(this.t * 6) % 4], 14, 2, 0.9, this.dir > 0); // crest art curls left
          else {
        ...
        case 'stalker': {
          ...
          const fr = AF('stalker', 4);
          if (fr) {
            // face the way it drifts; parchment art walks to the left
            if (this._lx !== undefined && Math.abs(this.x - this._lx) > 0.2) this._face = this.x > this._lx ? 1 : -1;
            this._lx = this.x;
            const img = fr[Math.floor(this.t * 4) % 4];
            const dh = 20, dw = dh * img.width / img.height;
            const dx = Math.round(this.x - dw / 2 - cam.x), dy = SY - dh;
            ctx.globalAlpha = 0.75;
            if ((this._face || -1) < 0) ctx.drawImage(img, dx, dy, Math.round(dw), dh);
            else {
              ctx.save();
              ctx.translate(dx + dw / 2, 0); ctx.scale(-1, 1); ctx.translate(-(dx + dw / 2), 0);
              ctx.drawImage(img, dx, dy, Math.round(dw), dh);
              ctx.restore();
            }
            ctx.globalAlpha = 1;
          } else {
